...
class VGMdbMetadataProcessor(object):
    ...
    def get_vgmdb_products(self, release_dict, response, strict_mode):
        ...
        for personnel_type in ['performers', 'composers']:
        ...
        if 'Various Artists' in source_artists:
            ...
            source_title = re.sub(pattern, '', release_dict['title'])
            vgmdb_titles = [re.sub(pattern, '', title.split(' / ', 1)[0]) for title in response['names']]
                ...
                for album in albums:
                    for title in album['titles']:
                        ...
                        if title.lower() == source_title:
                            ...
                            return vgmdb_id
                # Just return the first album if no exact match is found
